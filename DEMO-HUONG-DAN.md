...
# Mục đích: dọn pod rác (Evicted/Failed/Succeeded) trong istio-system & dev cho ảnh chụp sạch
...
```bash
export KUBECONFIG=/home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps/teammate-kubeconfig.yaml
# Mục đích: xác nhận kubectl đang trỏ đúng cụm DOKS trước khi demo
...
```bash
# Mục đích: kiểm tra /etc/hosts đã trỏ tên miền về IP Load Balancer chưa
...
# Mục đích: health check tổng thể (node / ArgoCD / pod dev-staging / endpoint) đầu buổi demo
echo "### Nodes"
...
# Mục đích: áp bộ policy mesh (mTLS STRICT + mTLS client + retry + authz) lên staging
...
```bash
# Mục đích: tạo pod test (mesh-tester) + httpbin retry — nguyên liệu cho kịch bản Mục 9
...
# Mục đích: đưa staging về trạng thái ban đầu (PERMISSIVE + gỡ mọi policy/pod demo)
...
```bash
cd /home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps
# Mục đích: tạo commit nhỏ trên nhánh feature để kích hoạt pipeline CI
...
```bash
# Mục đích: theo dõi pipeline CI ngay trên terminal (song song với web GitHub)
...
```bash
# Mục đích: xác minh image đã push lên GHCR với tag = commit SHA vừa push
...
```bash
# Mục đích: chạy CD Developer — deploy tax từ nhánh demo_ci_tax vào ns yas-developer
...
```bash
# Mục đích: kiểm chứng tax dùng image tag=SHA của nhánh, product dùng :latest
...
```bash
# Mục đích: xem commit "chore(dev): update image tags..." do workflow tự tạo (bằng chứng GitOps)
git log --oneline -5
...
```bash
# Mục đích: xem lịch sử các lần CD dev (auto-deploy mỗi khi merge vào main)
...
```bash
cd /home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps
# Mục đích: tạo & push tag semver v* để kích hoạt luồng CD staging
...
```bash
# Mục đích: kiểm chứng image staging mang tag = tên tag (v1.2.3), KHÔNG phải SHA
...
```bash
# Mục đích: xem ánh xạ tên miền *.yas.local.com → IP Load Balancer
...
```bash
LB_IP=129.212.208.194
# Mục đích: kiểm tra HTTP status khi vào qua tên miền (chứng minh Ingress route đúng)
...
```bash
export KUBECONFIG=/home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps/teammate-kubeconfig.yaml
# Mục đích: quét status code của TẤT CẢ tên miền một lượt (bảng tổng quan routing)
...
```bash
# Mục đích: chạy job cleanup — xóa toàn bộ namespace tạm yas-developer để trả tài nguyên
...
```bash
# Mục đích: xác nhận namespace tạm đã bị xóa sạch (đã trả RAM/CPU về cụm)
...
```bash
# Mục đích: chứng minh dev/staging đi GitOps (0 dòng helm), chỉ developer gọi helm thẳng lên cụm
...
```bash
export KUBECONFIG=/home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps/teammate-kubeconfig.yaml
# Mục đích: lấy mật khẩu admin để đăng nhập UI ArgoCD
...
```bash
# Mục đích: demo Self-Healing — sửa "chui" replicas rồi xem ArgoCD tự revert về Git
...
```bash
LB_IP=129.212.208.194
# Mục đích: sinh traffic để đồ thị Kiali có dữ liệu (Kiali đọc metric ~vài phút gần nhất)
...
```bash
# Mục đích: mở Kiali qua port-forward khi DNS/ingress trục trặc
...
```bash
# Mục đích: Authz ALLOW — SA 'order' nằm trong allow-list nên được vào product (KHÔNG 403)
...
```bash
# Mục đích: Authz DENY — SA 'default' KHÔNG trong allow-list → bị chặn 403 "RBAC: access denied"
...
```bash
# Mục đích: xác nhận phía server đã từ chối (log rbac_access_denied của Envoy tại product)
...
```bash
# Mục đích: Retry — gọi endpoint luôn trả 500 để chứng minh VirtualService tự retry 3 lần
...
```bash
# Mục đích: đọc số lần retry thực tế từ Envoy stats (upstream_rq_retry)
...
```bash
# Mục đích: xem cấu hình retry (attempts, retryOn) trong VirtualService
...
>
> **Trạng thái thật cần nói trước với thầy (cập nhật 2026-07-12):**
> - **Metrics: ✅ LIVE** — click thật trên Grafana.
> - **Traces: ✅ LIVE** — đã fix. Gốc lỗi cũ: OTel Java agent `2.6.0` không
>   instrument được app chạy **Java 25** nên KHÔNG sinh span nào (JVM metrics vẫn
>   chạy vì lấy qua MXBean). Đã nâng agent lên **`2.29.0`** (Instrumentation CR
>   `yas-instrumentation` + `k8s/deploy/observability/opentelemetry/templates/instrumentation.yaml`)
>   và restart pod → Tempo giờ có trace thật (product/cart/tax/order/inventory/BFF).
> - **Logs: ✅ LIVE** — đã fix. Gốc lỗi cũ: Loki chạy SimpleScalable nhưng thiếu
>   object storage (minio off, persistence emptyDir) → `read-only file system`,
>   không flush được chunk. Đã chuyển sang **SingleBinary + filesystem trên PVC**
>   (`k8s/deploy/observability/loki.values.fixed.yaml`, `helm upgrade` rev 9;
>   runbook `docs/loki-fix-runbook.md`). Verify: query `{namespace="staging"}` trả
>   log thật; Grafana → Explore → Loki HTTP 200.
...
```bash
export KUBECONFIG=/home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps/teammate-kubeconfig.yaml
# Mục đích: xem cấu hình OTel Collector — receivers OTLP rồi fan-out sang Tempo/Loki/Prometheus
...
| **Traces** | ✅ **LIVE** — đã nâng agent `2.6.0→2.29.0` (fix Java 25); collector nhận span (`receiver_accepted_spans>0`) → `tempo:4318`; Tempo có trace thật | **Click thật** trên Grafana → Explore → Tempo |
| **Logs** | ✅ **LIVE** — đã chuyển Loki sang SingleBinary + filesystem trên PVC (`loki.values.fixed.yaml`, rev 9); promtail đẩy log, query `{namespace=...}` trả data thật | **Click thật** trên Grafana → Explore → Loki (LogQL) |
...
Menu trái → **Dashboards** → mở **JVM (OpenTelemetry)** (uid `jvm-otel`). Ở thanh
biến trên cùng chọn **Namespace** (`staging`/`dev`) rồi **Service** (`product`,
`order`, `tax`, ...). Chỉ cho thầy và giải thích:

> ⚠️ **KHÔNG mở dashboard "JVM (Micrometer)" cũ (#4701)** — nó lọc theo label
> Micrometer (`application`/`area`) mà metric OTel không có, nên luôn **No data**.
> Nhóm đã thay bằng **JVM (OpenTelemetry)** dùng đúng label `service_name`/
> `jvm_memory_type`/`k8s_namespace_name` (file `k8s/deploy/observability/grafana/
> jvm_otel_dashboard.json`). Đây chính là lý do trước đó dashboard trống.
...
```promql
# Mục đích: gõ thử PromQL trong Explore (nhãn kiểu OTel: k8s_namespace_name, service_name)
...
> ✅ **Đã LIVE.** Sinh traffic rồi kiểm chứng có span:
> ```bash
 # Mục đích: sinh traffic rồi kiểm chứng Tempo đã nhận span thật
> LB_IP=129.212.208.194
> for i in $(seq 1 25); do curl -s -o /dev/null -H 'Host: storefront.staging.yas.local.com' http://$LB_IP/; done
> kubectl exec -n observability tempo-0 -- wget -qO- 'http://localhost:3200/api/search?limit=20'
> #   -> trả về danh sách trace (product/cart/tax/order/inventory/BFF...)
> ```
> Trên Grafana: **Explore → datasource Tempo → Search** (Service Name, Duration,
> Status=error) hoặc dán Trace ID để mở waterfall. Nếu Tempo trống, chỉ cần bắn
> thêm traffic (Tempo local storage, retention 24h).
...
```bash
# Mục đích: xem cấu hình correlation có sẵn (trace→log & log→trace) trong datasource
...
### 10.5-DEMO — Tự tạo 1 request LỖI để demo luồng trace → log (chạy live cho thầy xem)

> **Mục tiêu:** dựng sẵn một request lỗi thật để có **trace `Status=error`** + **dòng
> log ERROR có `traceId`** — nguyên liệu để bấm đúng luồng 5 bước ở 10.5. Chạy khối
> này **ngay trước khi vào Mục 10.5** để dữ liệu còn "nóng" (Tempo retention 24h).

**Bước 1 — Bắn request lỗi vào một service ĐƯỢC instrument (product):**
Gọi một sản phẩm **không tồn tại** → service `product` ném exception → OTel agent
sinh **span `status=error`** + app ghi **log ERROR kèm `traceId`**.

```bash
export KUBECONFIG=/home/huy/Documents/University/DevOps/Project02/Y3S2-DevOps/teammate-kubeconfig.yaml
LB_IP=129.212.208.194

# Mục đích: tạo lỗi 4xx/5xx CÓ TRACE — gọi API sản phẩm không tồn tại nhiều lần
for i in $(seq 1 15); do
  curl -s -o /dev/null -w 'attempt %s -> HTTP %{http_code}\n' "$i" \
    -H 'Host: api.staging.yas.local.com' \
    "http://$LB_IP/product/storefront/products/slug/loi-demo-$RANDOM"
done
```

> Không quan trọng chính xác mã trả về (404/500) — điểm mấu chốt là **có 1 SERVER
> span bị đánh dấu `error` + 1 dòng log ERROR mang cùng `traceId`**. Nếu route trên
> đổi tên, **bất kỳ request nào trả 4xx/5xx tới một service Spring Boot** đều dùng được
> (vd bỏ trống `/actuator`, sai path...). Muốn chắc chắn 500, có thể dùng thêm mục 9C
> (`httpbin /status/500`) — nhưng httpbin **không** được instrument nên **không sinh span**;
> vì vậy demo trace→log **phải bắn vào service app** như trên.

**Bước 2 — Xác nhận đã có trace lỗi trong Tempo (không cần biết trước traceId):**

```bash
# Mục đích: kiểm chứng Tempo đã ghi nhận span có status=error
kubectl exec -n observability tempo-0 -- \
  wget -qO- 'http://localhost:3200/api/search?tags=status.code%3Derror&limit=5'
# -> trả về danh sách trace lỗi; copy 1 "traceID" để dùng ở Bước 4 (hoặc lấy trên Grafana)
```

**Bước 3 — Trên Grafana, đi đúng luồng vàng 10.5 cho thầy xem:**
1. **Explore → datasource Tempo → Search**: đặt `Status = error` → chạy → mở 1 trace.
2. Nhìn **waterfall**: span **đỏ** nằm ở service `product` → "đã khoanh vùng được bug".
3. Copy **`traceId`** ở đầu trace (hoặc click span → nút **"Logs for this span"** =
   tính năng *trace-to-logs*) → Grafana tự mở **Loki** đã lọc đúng request đó.

**Bước 4 — Xem đúng dòng log lỗi của request đó (LogQL):**
Trên Grafana → Explore → **Loki**, dán (thay `<traceId>` bằng giá trị vừa copy):

```logql
{k8s_namespace_name="staging", service_name="product"} | json | traceId="<traceId>"
```

→ hiện **đúng dòng log ERROR + stack trace** của request lỗi (không phải mò cả biển log).

Đối chiếu nhanh bằng CLI (tùy chọn, thay `<traceId>`):

```bash
# Mục đích: chứng minh cùng traceId xuất hiện trong log của pod product
kubectl logs -n staging deploy/product --tail=500 | grep '<traceId>'
```

> **Câu chốt với thầy:** "Từ **1 triệu chứng lỗi**, em vào Tempo tìm trace `error`,
> waterfall chỉ ngay span đỏ ở `product`, rồi bấm *trace-to-logs* nhảy thẳng sang Loki
> lọc đúng `traceId` để đọc stack trace — đó là giá trị của observability hợp nhất:
> **metric báo *có vấn đề* → trace chỉ *ở đâu* → log nói *vì sao*.**"
...
```bash
# Mục đích: kiểm tra thành phần cần khôi phục nếu muốn bật lại Traces/Logs live
...
# Mục đích: hạ node pool xuống 2 để tiết kiệm chi phí (~$24/tháng/node) — chỉ khi không demo nữa
